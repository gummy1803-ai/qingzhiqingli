...
def render_performance_filter() -> dict:
    ...
    # 四列横向布局:车辆 / 起始时间 / 结束时间 / 电流目标点+持续时长
    ...
    logger.info("性能筛选: vehicle=%s range=[%s, %s] points=%d min_dur=%ds valid=%s",
                vehicle_id, start_datetime, end_datetime,
                len(current_points), min_duration, valid)
